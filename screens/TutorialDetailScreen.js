...
export default function TutorialDetailScreen({ tutorialId }) {
  ...
  useEffect(() => {
    const loadDetail = async () => {
      try { setLoading(true); setError(''); setTutorial(await getTutorialById(tutorialId)); }
      catch (e) { setError(e.message || 'Unable to load tutorial detail.'); setTutorial(null); }
  ...
  const onShare = async () => {
    ...
    const message = `Tutorial: ${tutorial.title}\nDifficulty: ${tutorial.difficulty}\nAverage time: ${tutorial.averageTime}\nDescription: ${tutorial.description}\nLink: https://ifn666.com/tutorials/${id}`;
